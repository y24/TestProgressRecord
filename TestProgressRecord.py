...
def aggregate_results(filepath:str):
    ...
    for sheet_name in sheet_names:
        ...
        for set in sets:
            ...
            # 環境ごとのデータ集計
            data_by_env[env_name] = DataAggregation.get_daily(data=set_data, filter=settings["common"]["results"])

    # 全セット集計(日付別)
    data_total = DataAggregation.get_daily(data=all_data, filter=settings["common"]["results"])
